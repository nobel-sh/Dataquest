...
import { Alert, Button, Card, MetricTile, Panel, Skeleton } from "@/components/ui/primitives";
    ...
    <Panel>
      <header className="border-b border-line p-7 max-sm:p-5">
        ...
        <div className="mt-4 grid grid-cols-3 border border-line text-sm max-sm:grid-cols-1">
          <MetricTile label="Total responses" value={responses.length.toString()} />
          <MetricTile
          ...
          <dl className="mt-4 grid gap-4">
            {fields.map((field) => (
              <div className="grid min-w-0 gap-1" key={field.id}>
                ...
                <dd className="m-0 break-words">{formatAnswer(response.answers[field.id], field)}</dd>
              </div>
            ))}
          </dl>
        </article>
